...
def psi(param,layers,N,J,h,PBC):
...
#state0T = state0.conjugate().T
#print('state0 shape ->\x09',state0.shape)
#print('state0.T shape ->\x09',state0T.shape)
#state1 = psi(param,layers,N,J,h,PBC)
#state1 = np.array([state1])
#state1T = state1.conjugate().T
#print('state1 shape ->\x09',state1.shape)
#print('state1.T shape ->\x09',state1T.shape)
#energy0 = np.dot(np.dot(state0,H),state0T).real
#energy1 = np.dot(np.dot(state1,H),state1T).real
#momentum0 = np.angle(np.dot(np.dot(state0,T),state0T))/np.pi
#momentum1 = np.angle(np.dot(np.dot(state1,T),state1T))/np.pi
#result = np.dot(state0,state1T)*np.dot(state1,state0T)
...
opt_param =[]
opt_energy = []
sys.stdout.write('{}'.format(base_term(param,layers,N,J,h,PBC,p,opt_param)))
#ret = minimize(base_term,param,args=(layers,N,J,h,PBC,p,opt_param),method='COBYLA',options={'maxiter': 10})
##print(type(ret.x))
#ret_seq = sequencial_minimizer(base_term,ret.x,Iter,layers,N,J,h,PBC,p,opt_param)
...
#    ret_seq = sequencial_minimizer(extra_term,ret.x,Iter,layers,N,J,h,PBC,p,opt_param)
#    opt_param_callbacks(ret_seq[0],opt_param)
#    opt_energy.append(ret_seq[1])
#    sys.stdout.write('opt_energy -> {}\n'.format(opt_energy))
#    sys.stdout.write('opt_param -> {}\n'.format(opt_param))
